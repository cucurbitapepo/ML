...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    ones = np.sum(target_sorted)

    y_sum = (np.cumsum(target_sorted)[:-1])[mask]
    left_count = (np.arange(1, total))[mask]
    right_count = (1 - np.arange(1, total))[mask]

    p1_left = y_sum / left_count
    p0_left = 1 - p1_left
    p1_right = (ones - y_sum) / right_count
    p0_right = 1 - p1_right

    gini_left = 1 - (p1_left ** 2 - p0_left ** 2)
    gini_right = 1 - (p1_right**2 - p0_right**2)

    ginis = -(left_count / total) * gini_left - (right_count / total) * gini_right

    best_ids = np.argmin(ginis)
    threshold_best = thresholds[best_ids]
    gini_best = ginis[best_ids]
